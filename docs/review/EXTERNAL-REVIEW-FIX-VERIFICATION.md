...
## 附录（2026-09-17）：E27 立场库第六批（知产）落盘（新设 `shared/positions/ip-positions.md` ＋ 知产岗指针 ＋ 验证记录）

> 本附录为**追加记录**，不改变前文第 1–5 节、两处补记与前述各附录的任何结论。
> 口径（硬）：只收知产岗 SKILL 各段现有口径**未覆盖**的项（逐条比对）；**不写法条编号与具体数字**（门槛、期限、比例、倍数一律改写为规则口径；条目序号用中文数字）；不搬原文整句，全部为中文重写，改写判据承接 E18 确立的**连续重合 ≤10 字**（去标点口径，书名署名行除外）；出处只到书名。
> 本批为立场库**第六批**，前五批（合同、劳动、建工、争议、治理）见前述五附录；六批合计建成 `shared/positions/` 目录下的六个文件。

### 一、本轮取材（署名）

| # | 书名 | 本批取材稿 | 落盘去向 |
| --- | --- | --- | --- |
| ① | 唐青林《商业秘密案件裁判规则》 | `.work/distill-outbox/25-mimi-caipan.md`【标准立场】节（只读） | `shared/positions/ip-positions.md` 各节，末注书名 |
| ② | 《商业秘密维权指南》 | `.work/distill-outbox/26-mimi-weiquan.md`【标准立场】节（只读） | 同文件对应条目，末注书名 |
| ③ | 《商业秘密保护实务及案例精解》 | `.work/distill-outbox/27-mimi-baohu.md`【标准立场】节（只读） | 同文件对应条目，末注书名 |
| ④ | 吴汉东《知识产权总论》 | `.work/distill-outbox/46-wuhandong.md`【标准立场】节（只读） | 同文件对应条目，末注书名 |

**署名来源与风险（必须写明）**：上表书名与作者的对应关系承接 **E15 附录第一节**（该节第 ④ 项唐青林《商业秘密案件裁判规则》、第 ⑤ 项王俊林等《商业秘密保护实务及案例精解》、第 ⑥ 项章建勤《商业秘密维权指南》）与 **E22 附录第一节**（吴汉东《知识产权总论》）已核实的登记，本批未另行推断。
- 稿内**自带来源行**：`25-mimi-caipan.md` 第 36 行载明「唐青林《商业秘密案件裁判规则》」（该稿第 1 行标题未载书目）；`27-mimi-baohu.md` 第 1 行标题载明书名，第 32 行载明 P2、P3、P4 三片的来源范围为该书第三至第六章及美/德/日/国际组织立法章节；`46-wuhandong.md` 第 1 行标题载明「吴汉东《知识产权总论》」，第 30 行载明 P5 片的来源范围为该书第十三章（后半）至第十七章。
- 稿内**未载来源行、书目依 E15 附录第一节已核实的对应关系登记**：`26-mimi-weiquan.md`（该稿第 34 行自述 P1～P8 八个碎片均未声明所覆盖的书目、章节、规则号或案例编号范围，逐片复核首行确无来源行，文中亦无书名出现；书目依 E15 附录第一节第 ⑥ 项登记，该项已注明**属推断、非草稿载明**）。
- **署名体例（如实登记）**：本批按简报「作者仅在草稿有载明时随附」的口径执行，故 `26-mimi-weiquan.md` 与 `27-mimi-baohu.md` 两稿所出条目**只写书名、不署作者**（两稿全稿检索「章建勤」「王俊林」各命中 0 处，作者在草稿中未载明）；`25-mimi-caipan.md`（第 36 行）与 `46-wuhandong.md`（第 1 行）两稿的作者在草稿中有载明，随附登记。两书的作者与书名的对应关系已由 E15 附录核实（其中⑥书为推断），本批未在立场库中登记作者，亦未另行推断。
- **本批未做**：四稿书目与作者的重新核实（承接 E15／E22 已核实的对应关系）；`25-mimi-caipan.md`、`27-mimi-baohu.md`、`46-wuhandong.md` 自述的碎片级缺口补做。
- 各稿自述的**碎片级缺口**（`25-mimi-caipan.md` 第 36–37 行载明仅 P1 声明范围、P1 末例未完整、P2～P10 九片未声明所覆盖的书目章节或规则号范围；`26-mimi-weiquan.md` 第 34 行载明八片均未声明来源范围；`27-mimi-baohu.md` 第 32–35 行载明 P1、P5、P6 三片未声明范围、无碎片声明覆盖原书第一、二章、P4 声明的案例范围止于第 10 号；`46-wuhandong.md` 第 30–31 行载明仅 P5 声明书目范围、九片未记录页码或章节边界）**本批未补做**，见第五节边界。

### 二、本轮落盘改动（3 个文件，全部在允许清单内）

| # | 文件 | 改动内容 | 性质 |
| --- | --- | --- | --- |
| ① | `shared/positions/ip-positions.md` | **新设文件**：知产立场库，共 50 条（限额 ≤50），按十三节分主题（商业秘密的客体与保护条件、客户名单与经营信息、保密措施与保密义务、密点撰写与同一性比对、侵权认定与归责、合法来源与反向工程、竞业限制与商业秘密的关系、举证与证据、刑民交叉与责任并行、责任方式与赔偿、知识产权的一般原理与权利范围、权利限制与滥用规制、国际保护与发展立场），每条固定「应然＋底线＋出处」三段，条目序号用中文数字连续编排，行内署名只到书名 | 生产文件；`shared/positions/` 目录下的第六个文件 |
| ② | `roles/ip-counsel/SKILL.md` | 第 3 段「本岗不内联法条汇编。相关检查点见：」列表内新增 1 行指针，指向立场库 | 生产文件；`git diff --numstat` 为 `1 0`（1 insertion），第 0–2 段、第 4–7 段既有条目一字未动，版本号与知识截至日期未改 |
| ③ | `docs/review/EXTERNAL-REVIEW-FIX-VERIFICATION.md` | 本附录 | 追加记录，不改前文 |

- **与前述各批的体例差异（如实登记）**：本批沿用**劳动、建工、争议、治理四批**的体例：`##` 级为不带序号的大类，条目号以 `### 一、` 至 `### 五十、` 逐条编号。六库的引用口径一致（产出中写「按知产立场库第〔条号〕条」并同时引用既有检查点编号与风险锚点）。
- **未提交、未推送**，改动全部留在工作区；基线提交 `f4735ae`（执行前实测 `git rev-parse --short HEAD`，实测值与简报所记一致）。
- 禁止触碰文件核对：`outputs/external-review-39abd04.md`、`.work/` 下全部历史文件与本次只读的 4 份取材稿、合同红线清单、证据固定清单、其他岗位文件、既有条目文字与编号均未改。

### 三、去重口径说明

- **唯一的收录标准**：知产岗 SKILL 各段（第 0 段三条硬规则、第 1 段分诊、第 2 段失败模式、第 3 段法律框架、第 4 段分析脚手架第 1–22 条、第 5–7 段交付物与红线）是否已经写过同一判断口径；已覆盖的一律不收录。
- **去重为程序化实测，非人工印象**：把 50 条条目的正文（「应然」行＋「底线」行，出处行不计入）与知产岗 SKILL 全文做**最长连续重合**测算（去标点口径），实测**最大 7 字、≥10 字者 0 条**（最大值片段「反不正当竞争法」，属该条主张的核心术语，无法再降）。
- 本批**因与知产岗 SKILL 重复、故未收录的典型项及覆盖位置**：
  - 密点须逐一点明范围与边界、主张过宽会削弱同一性认定、主张不明会被认定披露不足——知产岗 SKILL 第 4 段第 9 条已有；
  - 秘密性的判断时点锁定在被诉行为发生时、此后的公开不影响此前秘密性的成立——第 4 段第 9 条已有；
  - 被诉行为发生后补做的保密措施不能回填——第 4 段第 9 条已有；
  - 区分对内措施与对外措施、两类措施不互相替代、措施须与被诉获取方式对应、对外措施须能对抗反向工程、产品标签只有表明保密目的才算数、安全警示与维修担保提示不构成保密措施——第 4 段第 10 条已有（本库第 八、十一 条只取「不必万无一失」与「与商业价值相适应且具体特定」两个未写过的角度）；
  - 员工知识、经验与技能不因来源于企业即归企业所有、须先与被主张信息作区分——第 4 段第 10 条已有；
  - 先提交已采取保密措施且被侵犯的初步证据、再要求对方就合法来源与公知性举证、不把责任转移理解为倒置而自己不举证——第 4 段第 12 条已有；
  - 鉴定只解决专业技术事实、不替代法律判断、结论模糊或未界定时间段的鉴定意见不列入采信基础——第 4 段第 12 条已有；
  - 许可类型决定起诉主体资格、被许可人不得越位单独起诉——第 4 段第 12 条已有；
  - 民事救济请求中不列恢复原状与返还原物、侵害对象表述为擅自行使或利用专有权利——第 4 段第 18 条已有；
  - 以过错责任为一般原则、损害赔偿以过错为条件、停止侵害一类物权性请求不以过错为条件、不得把赔偿请求一概套用无过错责任——第 4 段第 19 条已有；
  - 区分知识产权的「存在」与「使用」、存在本身属竞争法除外领域、只有使用构成滥用才受反不正当竞争法规制、反不正当竞争法是行为规制法而非权利法、不作为独立诉权基础只作派生救济——第 4 段第 20 条已有（本库第 四十六、四十七 条只取「滥用交由反垄断法处理」与「拥有权利不推定市场支配力」两个未写过的角度）；
  - 核对是否存在公共领域保留、保护期届满与不具备实质要件的知识产品、人类共有的知识、专有权扩张类争点须识别公共领域是否被压缩——第 4 段第 21 条已有；
  - 国民待遇、最低保护标准、公共利益三项原则各自的射程——第 4 段第 22 条已有（本库第 四十九 条只取「保护水平与本国发展阶段相适应」这一未写过的角度）；
  - 以「知识产品」为落点、不用「物」或「智力成果」、逐项核对客观性对象性与可支配性、与其他财产权的分野落在客体的非物质性上——第 4 段第 13 条已有（本库第 三十八 条只取「三大基本特点与公开性的例外」这一未写过的角度）；
  - 遇「新型财产权」主张先归入创造性成果、经营性标记、经营性资信这一组类别、归不进去的再讨论是否另立权利——第 4 段第 14 条已有（本库第 四十四、五十 条只取「发现权发明权不宜纳入」与「遗传资源权的定位」两个未写过的角度）；
  - 原始取得须同时具备创造者的创造性行为与国家机关的授权性行为、权利的实现走创造传播使用这条链、继受取得的核查与原始取得同等重要——第 4 段第 15 条已有；
  - 把利用分为约定利用与法定利用、分别核对适用范围与意思自治程度与主体、分清所涉限制是行使效力层面还是存在与存续效力层面——第 4 段第 16 条已有（本库第 三十九 条只取「利用的对象是权利而非知识产品」这一未写过的角度）；
  - 讨论平行进口前先确定权利穷竭的效力范围是国内还是国际、出租权与进口权一类设计是对穷竭的反限制——第 4 段第 17 条已有；
  - 不核权利状态就给出维权或防御方案、把持有权利证书等同于权利稳定、商标维权不看核定类别与地域直接跨类主张、专利分析只谈侵权不谈保护范围解释与稳定性、著作权归属忽略职务作品与委托创作的约定优先、商业秘密方案只写保密条款不给密点固定与访问分级与留痕与离职交接、只给「可以起诉」不给证据固定与公证与时间戳路径、忽略平台投诉与行政投诉与海关备案等非诉快速通道、许可谈判只谈价格不谈范围与地域与期限与排他性与改进成果归属与终止后库存——第 2 段失败模式已有；
  - 不得建议抢注他人商标、恶意投诉、虚假标注专利号或权利状态、不得以非法手段获取对方技术资料与客户名单、不得承诺侵权认定结论与无效结果与赔偿金额、不得以「先使用后补登记」「先公开后申请」掩盖新颖性丧失或在先使用的真实风险、不得输出量化风险分或颜色词等级——第 7 段红线与第 0 段三条硬规则已有；
  - 涉嫌侵犯商业秘密与假冒注册商标等刑事风险的转介、无效宣告与商标评审程序代理的转介、涉外权利程序的转介、职务发明奖励报酬与离职员工保密竞业安排的用工侧协同、已进入侵权诉讼或仲裁的转回——第 6 段升级条件与第 0 段第 3 条已有（本库第 三十、三十一、三十二 条只取「刑事路径优先选公诉」「民事责任不因刑事追责而免除」「刑事裁判对后续民事的既判力是相对的」三个未写过的角度）。
- **本批有意不落的项**：取材稿中带条号、门槛、期限、比例、倍数与地方口径列举的条目（如诉讼时效年数、举证责任转移的条文版本与款项层级、密点撰写所参照的专利审查基准章节、定罪量刑与立案追诉的数额档、罚金倍数与法定赔偿上限、惩罚性赔偿的倍数档与技术贡献率百分比、竞业限制的年限档与经济补偿口径、行政罚款的幅度区间、涉外立法的条号与刑期与罚金额、条约的条号列举等），按硬口径不移植；纯境外程序法与外国法规则（美国商业秘密法的扣押与禁令与惩罚性赔偿与披露免责、德国与日本与欧盟的立法条文、生物多样性公约与知识产权协定的条文冲突列举等），超出「主张强度」范围，本轮未收，改由「按涉外权利程序转介」提示承载；纯学理争议条目（知识产权的私权定性与人权位阶之争、知识产权法的部门法归属与法典化体例之争、商业秘密制度的思想基础嬗变、知识产权文化的移植与再造路径、遗传资源权与知识产权关系的诸说等），不属主张强度口径，本轮未收（本库只保留其中已形成通说、可直接用于主张强度的少数条目）。

### 四、本批自验实测（静态）

判据承接 E18／E23／E24／E25／E26：被测条目去掉末尾出处署名行后的正文，与草稿**两侧同口径**处理（去空白与标点，仅保留汉字、字母与数字），求**连续**相同字符串的最大长度；**达到或超过 10 字判为需要改写**。

```
被测条目：shared/positions/ip-positions.md 全部 50 条（每条＝「应然」行＋「底线」行，出处行不计入）
比对草稿：.work/distill-outbox/ 下 4 份取材稿全文（25-mimi-caipan、26-mimi-weiquan、27-mimi-baohu、46-wuhandong）
比对池字符数（去标点口径）：25-mimi-caipan 19073、26-mimi-weiquan 13199、27-mimi-baohu 12942、46-wuhandong 17091
实测结果：最大连续重合 9 字，≥10 字者 0 条
          （并列最大值 9 字见于 4 条：第 二十 条「负有保密义务的员工」、第 三十三 条「为公众所知悉时为止」、
            第 四十三 条「效力取决于保密程度」、第 四十六 条「知识产权滥用的规制」，
            属各该条主张的核心术语，无法再降）
          （改写过程如实登记：初稿自检最大 26 字、≥10 字者 18 条；首轮逐条改写后降至最大 14 字、≥10 字者 2 条；
            再经一轮改写复跑至最大 9 字、超标 0 条）
条目计数：50 条（限额 ≤50）
三要素：应然／底线／出处齐备 50／50
```
...
```
对知产岗 SKILL 的去重实测（同一判据，比对池为 SKILL 全文）：
比对池：roles/ip-counsel/SKILL.md（4449 字符）
实测结果：最大连续重合 7 字、≥10 字者 0 条（片段「反不正当竞争法」）
```

```
原文整句复制检查（草稿按句号、分号、换行切句，归一化后长度 ≥15 字的句子逐一回查立场库全文）：
整句命中数 = 0
```
...
阿拉伯数字命中: 0
百分号命中: 0
金额单位命中: 0（宽口径扫描曾命中「万」1 处，核对系「万无一失」的成语词素，
  非金额数值，不计入；逐处复核无门槛、期限、比例、倍数类汉字数字）
（出处行 50 行为书名署名行，按硬口径豁免，不计入上述统计）
```
...
git diff --name-only →
  docs/review/EXTERNAL-REVIEW-FIX-VERIFICATION.md
  roles/ip-counsel/SKILL.md
git status --porcelain →
   M docs/review/EXTERNAL-REVIEW-FIX-VERIFICATION.md
   M roles/ip-counsel/SKILL.md
  ?? .workbuddy-ai/            （任务开始前既有，未动）
  ?? shared/positions/ip-positions.md   （本批新设）
git rev-parse --short HEAD → f4735ae（会话内未提交，基线未变，实测值）
roles/ip-counsel/SKILL.md 的 git diff --numstat → 1  0
Select-String 'E27' docs/review/EXTERNAL-REVIEW-FIX-VERIFICATION.md → 有命中（本附录标题行）
```

### 五、边界

- 本附录记录的是**文本与结构落盘 ＋ 静态自验**，与第 0 节同一口径：不证明宿主在真实会话中会据此产生预期行为（新增条目是否会被真的逐项引用、指针是否会被真的读取，均需宿主实测）。
- **未做**：四本原书全本通读（只读四份合并稿的【标准立场】节）；四稿自述缺口未补——`25-mimi-caipan.md` 第 36–37 行载明仅 P1 声明范围、P1 末例未完整、P2～P10 九片未声明范围；`26-mimi-weiquan.md` 第 34 行载明 P1～P8 八片均未声明来源范围；`27-mimi-baohu.md` 第 32–35 行载明 P1、P5、P6 三片未声明范围、原书第一二章无碎片声明覆盖、P4 声明的案例范围止于第 10 号；`46-wuhandong.md` 第 30–31 行载明仅 P5 声明书目范围、九片未记录页码与章节边界——均照稿登记，未作推断补写；【标准立场】节条目的全量落盘（本批按未覆盖口径精选，四稿【标准立场】合计实测 432 条中收录 50 条）；草稿所涉法条编号、门槛、期限与比例数字的联网核验（按硬口径不移植，故无核验对象）；书中内容在中国法下的实体正确性复核；四稿【检查项】【误判教训】两类的落盘（不在本批取材范围）。
- 四份取材稿与 `.work/` 下历史文件为**只读输入**，本批未修改。

---

## 附录（2026-09-17）：E28 `industries/construction/regulations.md` 第 10 行归属订正（释义核验遗留修复）

> 本附录为**追加记录**，不改变前文第 1–5 节、两处补记与前述各附录的任何结论。
> 口径（硬）：只改该行归属表述，**不增删规则实质**；**不写法条编号与具体数字**（期限数字不写，条目序号用中文数字；本附录同样不重述条号与期限数字，只给文件行号与底本行号）；**不搬释义原文整句**，改写判据承接 E18 确立的**连续重合 ≤10 字**（去标点口径）；释义底本引述只给行号，不给整句。

### 一、问题来源与订正口径

- 问题由前述「民法典释义核验底本 · 生产文件条号引用存在性核验」附录**第七节**记录（该节标题即「超出三态的备注（1 条，不计入三态统计，只记录不改写）」）。该节按当时简报口径**只记录、未改写**；本附录即按「发现错误另开修复任务」立项后的修复记录。
- 该节的认定：生产文件该行**要点栏**把「行使期限有法定限制，逾期失权」写在民法典该条文行内；而释义底本把优先受偿权的行使期限列为立法中的**争议问题**（底本 L15390，该段标题即「优先受偿权的行使期限」，段内列举了主张设固定期限与反对设期限两方观点），即该民法典条文**未规定**该期限；底本全文检索该期限数字命中 **0**。
- 故该「法定限制」不能挂在民法典该条文名下；其规范来源即该行**状态备注自述**的两部司法解释条号（解释一优先受偿权行使期限条、解释二起算规则条）。按本附录硬口径，上述条号与期限数字在此不重述，位置见该行状态备注原文与上述核验附录第七节。
- **订正口径（一句话）**：只订正归属表述——把期限来源指向司法解释、并明示该民法典条文未规定该期限；**规则实质不变**（期限受限制、逾期失权这一判断保留），该行其余文字与全表其他行一字未动。

### 二、本轮改动（2 个文件，全部在允许清单内）

| # | 文件 | 改动内容 | 性质 |
| --- | --- | --- | --- |
| ① | `industries/construction/regulations.md` | 第 10 行**要点栏一处**归属表述订正：期限来源改为指向司法解释，并明示该民法典条文未规定该期限 | 生产文件；`git diff --numstat` 为 `1 1`（1 insertion ＋ 1 deletion）；同行其余文字、同表其他各行、文末声明块一字未动 |
| ② | `docs/review/EXTERNAL-REVIEW-FIX-VERIFICATION.md` | 本附录 | 追加记录，不改前文 |

- 订正前后对照（仅该分句，其余同）：
  - 订正前：`行使期限有法定限制，逾期失权`（把期限挂在该民法典条文名下）
  - 订正后：`行使期限的限制另由司法解释规定（本条未规定该期限），逾期失权`
- 规则实质比对（逐点）：两版均保留「行使期限受限制」与「逾期失权」两个判断，未增删规则、未新增例外、未改法律效果；改动只落在**限制的规范来源**这一归属表述上。
- 行内条号与期限数字：该行状态备注原有内容（含两部司法解释条号与期限数字）**未动**；要点栏订正后**未新增**任何条号与具体数字（实测见 §三）。
- **未提交、未推送**，改动全部留在工作区；基线提交 `f01c4a5`（执行前实测 `git rev-parse --short HEAD`，实测值与简报所记一致）。
- 禁止触碰文件核对：`outputs/external-review-39abd04.md`、`.work/` 下全部文件（含只读的释义底本）、其他岗位／清单／模板文件、既有条目文字与编号均未改。

### 三、本批自验实测（静态）

判据承接 E18／E23／E24／E25／E26／E27：被测文本与比对池**两侧同口径**处理（去空白与标点，仅保留汉字、字母与数字），求**连续**相同字符串的最大长度；**达到或超过 10 字判为需要改写**。

```
被测文本：industries/construction/regulations.md 第 10 行要点栏订正后新增分句
          （归一化后 27 字）
比对池  ：.work/distill-inbox/weread-exporter/output/中华人民共和国民法典释义及适用指南.md
          全文（只读输入，本批未修改）
实测结果：最大连续重合 8 字、≥10 字者 0 条
          （最大值片段经原始上下文回查，仅 1 处命中，系底本另一章释义中
            句读处相邻形成的巧合字串，语义无关，无法再降）
```

```
原文整句复制检查（底本按句号、分号、换行切句，归一化后长度 ≥15 字的句子
共 35140 句，逐句回查新增分句全文及其 ≥15 字前缀）：
整句命中数 = 0
```
...
「第…条」式条号命中: 0
阿拉伯数字命中: 0
汉字数字命中: 0（逐处复核无门槛、期限、比例、倍数类汉字数字）
```

```
改动范围与基线：
git diff --name-only →
  industries/construction/regulations.md
git status --porcelain →
   M industries/construction/regulations.md
  ?? .workbuddy-ai/            （任务开始前既有，未动）
git diff --numstat → 1  1  industries/construction/regulations.md
git rev-parse --short HEAD → f01c4a5（会话内未提交，基线未变，实测值）
git log -1 --format=%H → f01c4a59dfa65994734bf5771b5fbbe7e47c62ab
Select-String 'E28' docs/review/EXTERNAL-REVIEW-FIX-VERIFICATION.md → 有命中（本附录标题行）
```
...
## 附录（2026-09-17）：E29 实战补强落盘（原合同缺失动作 / 自然人主体 / 共管与私户 / 验证记录）

> 本附录为**追加记录**，不改变前文第 1–5 节、两处补记与前述各附录的任何结论。
> 口径（硬）：只做三处实战缺口的补强；**新增条目正文不写法条编号与具体数字**（门槛、期限、比例一律改写为规则口径）；**不搬原文整句**，全部为中文重写；新增条目与既有体例一致。
> 来源：实战审查复盘（三处覆盖缺口）＋ 本轮落盘表。本轮**未引入任何外部来源**，故不做许可核验、不新增署名。

### 一、来源与口径

- 缺口来源：一次劳务合作协议的实战审查复盘结论，指出合同岗与两份清单存在三处覆盖缺口——①对方援引我方未见的在先合同作为本件依据；②签约方为自然人时的签署形式与责任形式；③工程款共管账户与走个人账户的安排。
- 三处缺口均经逐条比对确认为**现有条目未覆盖**，落点分别为：合同岗第 1 段「收集必备事实」清单、合同红线清单 A 节（主体与效力）、建工施工合同清单的「未覆盖项」序列。
- 本批**未新引入来源**：三处新增均为对实战审查口径的中文重写，未复制任何上游、出版物或他人文本整句；故本批不做许可核验、不新增署名。

### 二、本轮落盘改动（4 个文件，全部在允许清单内）

| # | 文件 | 改动内容 | 性质 |
| --- | --- | --- | --- |
| ① | `roles/contract-counsel/SKILL.md` | 第 1 段第 2 项「收集必备事实」清单**末位新增 1 条**：对方援引我方未见的在先合同作为本件权利义务依据时，以提供全文并书面锁定适用范围为签字先决条件、并给出对应标准表述；拿不到全文的转律师审查，不得按未知文本承担义务 | 生产文件；只加 1 条子项，未改第 1 段其余条目，未改第 0、2–7 段一字 |
| ② | `shared/checklists/contract-redlines.md` | A 节**新增 A6 1 条**（含「标准立场」子项）：签约方为自然人时的证件核验与留存、本人签字并按指印、签署身份与所负责任形式写清，并提示家事财产混同与家庭成员连带风险 | 生产文件；只加 1 条，未改 A1–A5 与其余各节一字 |
| ③ | `industries/construction/checklists/construction-contract.md` | 清单**新增 2 条「未覆盖项」**：工程款共管账户（联签门槛与例外＋对账机制＋专款专用与禁止挪用＋共管形同虚设的写法）；工程款走个人账户或个人名义收付（税务风险＋挪用风险＋改为对公账户并留存凭证） | 生产文件；只加 2 条，未改既有各条一字 |
| ④ | `docs/review/EXTERNAL-REVIEW-FIX-VERIFICATION.md` | 本附录 | 追加记录，不改前文 |

- 生产文件改动数：**3 个**（第 4 个为本验证记录）。
- 新增条数：合同岗 **1 条**、红线清单 **1 条**、建工清单 **2 条**，合计 **4 条**，与简报「4 处新增」一致。
- 禁止触碰文件核对：`outputs/external-review-39abd04.md` 未改；`.work/` 下全部文件（含本次只读的简报）未改；其他岗位／清单／模板文件未改；既有条目文字与编号未改；F 盘其他项目未改。
- **未提交、未推送**，改动全部留在工作区；基线提交 `2f398cb`（执行前实测 `git rev-parse --short HEAD`，实测值与简报所记一致）。

### 三、简报验收命令与真实输出（逐条复跑，2026-09-17）

**验收 1（合同岗新增原合同缺失动作 1 条）**

```powershell
Select-String -Path 'roles/contract-counsel/SKILL.md' -Pattern '原合同'
# → 33:    - 对方援引我方未见的在先合同（原合同、主合同、框架协议或补充协议）作为本件权利义务依据的：…
Select-String -Path 'roles/contract-counsel/SKILL.md' -Pattern '原合同' | Measure-Object
# → 工作区 Count = 1
(git show HEAD:roles/contract-counsel/SKILL.md) | Select-String -Pattern '原合同' | Measure-Object
# → 基线 Count = 0
```

结论：**通过**。命中 1 处且为新增行（同一文件同一模式在基线的命中数为 0）；落点在**第 1 段第 2 项「收集必备事实」**清单末位（该清单原有 5 条，现为 6 条），未改第 1 段其余条目。

**验收 2（红线清单新增自然人主体 1 条；建工清单新增共管/私户 2 条）**

```powershell
Select-String -Path 'shared/checklists/contract-redlines.md' -Pattern '^- A6'
# → 19: - A6 签约方为自然人时：是否核验并留存其身份证件信息（证件号码与复印件）、是否由本人签字并按指印、…
Select-String -Path 'industries/construction/checklists/construction-contract.md' -Pattern '共管账户|个人账户'
# → 45: - [ ] （未覆盖项）工程款共管账户：…
# → 46: - [ ] （未覆盖项）工程款走个人账户或个人名义收付：…
```

计数（基线 ↔ 工作区，同口径实测）：

```
redlines A 节条目数       : 基线 5  → 工作区 6   （净增 1，即 A6）
construction 未覆盖项条数 : 基线 30 → 工作区 32  （净增 2，即共管账户、个人账户两条）
```

结论：**通过**。红线清单新增 **1** 条（A6，含「标准立场」子项 1 处，与 A1–A5 体例一致）；建工清单新增 **2** 条（均置于既有「未覆盖项」序列末位，体例一致）。

**验收 3（E29 附录在验证记录，含实战来源说明）**

```powershell
Select-String -Path 'docs/review/EXTERNAL-REVIEW-FIX-VERIFICATION.md' -Pattern 'E29'
# → 有命中（本附录标题行）
```

结论：**通过**。附录含来源说明（第一节）、落盘表（第二节）、逐条自验输出（第三节）、口径自验（第四节）与边界（第五节）。
...
# → roles/contract-counsel/SKILL.md
# → shared/checklists/contract-redlines.md
git diff --numstat -- roles/contract-counsel/SKILL.md shared/checklists/contract-redlines.md industries/construction/checklists/construction-contract.md
# → 2  0  industries/construction/checklists/construction-contract.md
# → 1  0  roles/contract-counsel/SKILL.md
# → 2  0  shared/checklists/contract-redlines.md
git status --porcelain
# →  M docs/review/EXTERNAL-REVIEW-FIX-VERIFICATION.md
# →  M industries/construction/checklists/construction-contract.md
# →  M roles/contract-counsel/SKILL.md
# →  M shared/checklists/contract-redlines.md
# → ?? .workbuddy-ai/          （任务开始前既有，未动）
git rev-parse --short HEAD → 2f398cb
git log -1 --format=%H → 2f398cbd191b1efe77d72ab51ae1fb5242f68f0b
```

- 上表为**追加本附录之后**的最终实测状态。追加前 `git diff --name-only` 只列 3 个生产文件，与本节验收 1、2 的输出时点一致；本附录自身的改动在追加后才进入 diff，故最终为 4 个文件。
- 三个生产文件的 `numstat` 删除数均为 **0**（纯新增，无既有文字被改），上表已按文件名限定复跑，该口径**不受本附录自身行数变动影响**。
- 本验证文件自身的 `numstat` 新增行数**不在此固定记录**：该数值随本附录文字每一次修订而变，写入即失真；其可核事实是「本附录为纯追加 ＋ 末尾一处换行补齐」（原文件末尾 `---` 无换行结尾，追加时按既有体例在 `---` 后补一空行再起附录标题，`git diff` 该处仅消除 `\ No newline at end of file` 标记，未删改任何既有文字）。

结论：**通过**。改动范围为**本附录第二节所列 4 个文件**（3 个生产文件 ＋ 本验证记录），全部在简报允许清单内；三个生产文件为纯新增（删除数 0）；`?? .workbuddy-ai/` 为**任务开始前既有**的未跟踪内容，本批未触碰。基线仍为 `2f398cb`，**未提交、未推送**。

### 四、口径自验（新增条目正文）

判据：新增条目正文中「第…条」式条号、阿拉伯数字、百分号与门槛／期限类数字的命中数。

```
新增文本行数 = 5（合同岗 1 行 ＋ 红线清单 2 行 ＋ 建工清单 2 行）
「第…条」式条号命中 : 0
阿拉伯数字命中       : 1（仅条目编号「A6」本身，非门槛或期限数字）
百分号命中           : 0
汉字数字命中         : 3 处，逐处回查均为普通用字（「一并评估」「一方单独」「不一致」），
                       无门槛、期限、比例、倍数类数字
```

结论：新增条目正文**不含任何法条编号与门槛／期限／比例数字**。说明：本附录第三节的自验输出含行号、条数与提交哈希，属验证记录所需，非条目内容。

**「不搬原文整句」自验**（判据承接 E18 确立的**连续重合 ≤10 字**：两侧同口径归一化，去空白与标点，仅留汉字、字母与数字，求连续相同字符串的最大长度）：

```
被测文本：本次 5 行新增文本全文
比对池  ：基线 HEAD 全部 .md 文件（git archive 解出到临时目录后扫描，实测 42 个文件；
          即新增之前的既有文本，不含本次改动，避免被测文本与自身比对）
实测结果：5 行新增文本的最大连续重合均为 0 字（<10 字判据），无需改写
```

结论：**通过**。新增文本与仓库既有全部 `.md` 文本无任何 ≥10 字的连续重合，且本批**未抓取任何上游或出版物原文**（来源为实战复盘结论），不存在原文整句搬运。

### 五、边界

- 本附录记录的是**文本与结构落盘 ＋ 静态自验**，与第 0 节同一口径：不证明宿主在真实会话中会据此产生预期行为（新增条目是否会被真的逐项核对、对方援引在先合同时是否真的会被要求提供全文，均需宿主实测）。
- **未做**：三处新增条目所涉规则的**条文级联网核验**（按硬口径不写条号，故无核验对象）；实战原案的完整卷宗复核（本批只据复盘结论落盘，未回看原案材料）；新增条目在类案裁判中的倾向性实证；合同岗新增条目与合同红线清单 A 节、`shared/positions/contract-positions.md` 的交叉去重（本批只做「既有条目是否已覆盖」的初步比对）。
- `.work/` 下历史文件与本次只读的简报为**只读输入**，本批未修改。
